...
async function doSync() {
  ...
  lastResult.value = ''
  try {
    const r = await runSync(getTransport(), settingsStore.webdav.maxBackups ?? 10, resolution.value)
    await refreshAfterDataChange()
    await refreshStatus()
      ...
      >
        <div class="bg-white rounded-xl border border-zinc-200 shadow-2xl w-full max-w-[520px] max-h-[85vh] flex flex-col text-[13px]">
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            <!-- Conflict items (expandable, default open) -->
          ...
          <div class="shrink-0 border-t border-zinc-100 p-3 flex gap-2">
            <UButton variant="secondary" size="md" class="flex-1" @click="onPreview('local')">
              偏向本地重算
            </UButton>
            <UButton variant="secondary" size="md" class="flex-1" @click="onPreview('remote')">
              偏向云端重算
            </UButton>
            <UButton
              variant="primary"
